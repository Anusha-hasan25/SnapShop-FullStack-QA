...
const SilverBracelets = () => {
  const { addToCart, toggleWishlist, wishlist } = useCart(); // Added wishlist
  useEffect(() => { window.scrollTo(0, 0); }, []);

  const products = [
    { id: 'sb-1', name: "Sterling Silver Bangle", price: 15000, image: "/images/silver-bangle.jpg", category: "Silver" },
    { id: 'sb-2', name: "Silver Link Bracelet", price: 9500, image: "/images/silver-link-bracelet.jpg", category: "Silver" },
  ];

  return (
    <div className="info-page-wrapper">
      <div className="info-container">
        <header className="info-header text-center pt-20"> {/* Spacing fix */}
          <h1 className="serif italic text-4xl mb-4">Silver Bracelets</h1>
          <p className="text-[11px] tracking-[4px] uppercase text-gray-900 font-bold">
            High-polish bangles and delicate links in 925 sterling silver
          </p>
        </header>
        <div className="product-grid mt-16">
          {products.map((item) => {
            const isFavorite = wishlist.some(fav => fav.id === item.id); // logic check
            return (
...
export default SilverBracelets;